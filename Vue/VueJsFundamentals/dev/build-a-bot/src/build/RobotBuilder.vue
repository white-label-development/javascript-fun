...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    if (this.addedToCart) {
      // don't allow abandon without warning

      /* eslint no-alert: 0 */
      /* eslint no-restricted-globals: 0 */
      const response = confirm('Really quit?');

      next(response);
    } else {
      next(true); // allow next
    }
  },
  ...
  created() {
    console.log('explicit lifecylehook fired (not the mixin version');
  },
  ...
  computed: {
    ...
    // selectedRobot() {
    //   //<img :src="selectedRobot.head.src" title="head"/>
    //   //replaces <img :src="availableParts.heads[selectedHeadIndex].src" title="head"/>
    //   return {
    //       //moved to PartsSelector.vue - initialized in data() as no longer a computed object.
...
.add-to-cart {
  position: absolute;
  // right: 30px; dropped when button moved into preview
  width: 210px;
  padding: 3px;
  font-size: 16px;
}
